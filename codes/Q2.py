...
data['Quarter'] = data['Month Won or Lost'].apply(lambda x: ((x.month-1)//3) + 1)
data['Quarter_New'] = data.apply(lambda row: str(row['Month Won or Lost'].year) + '-' + 'Q' + str(row['Quarter']).split('.')[0], axis=1)
z = data.groupby(['Quarter_New', 'Contract Status'], as_index=False)['Hospital'].count()
zz = pd.pivot_table(z, index='Quarter_New', columns='Contract Status', values='Hospital', aggfunc='sum').reset_index().rename_axis(None, axis=1)
zz = zz.fillna(0)
zz.columns = ['Quarter', 'Hospitals_Lost', 'Hospitals_Won']
...
fig.update_layout(
    title='Hospitals Won and Lost with Net Hospitals Won (Rolling)',
...
fig.update_traces(
    selector=dict(name="Net Hospitals Won"),
