...
z = data.groupby(['Month Won or Lost', 'Contract Status'], as_index=False)['Total Births'].sum()
zz = pd.pivot_table(z, index='Month Won or Lost', columns='Contract Status', values='Total Births', aggfunc='sum').reset_index().rename_axis(None, axis=1)
zz['Month Won or Lost'] = zz['Month Won or Lost'].apply(lambda x: str(x)[:7])
zz = zz.fillna(0)
zz.columns = ['Month', 'Births_Lost', 'Births_Won']
...
line_chart = go.Scatter(
    ...
    textfont=dict(color='hotpink', size=7)  # Text color for the line chart labels
)
...
fig.update_layout(
    title='Births Won and Lost with Net Births Won (Rolling)',
